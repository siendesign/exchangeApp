...
export async function POST(
  ...
  try {
    ...
    const newChat = new Chat({ orderId, message, from, to, type });
    ...
    return NextResponse.json(orderID + "mail sent", { status: 200 });
  } catch (error) {
...
export async function GET(
  ...
  try {
    db.connect();
    const chats = await Chat.find({orderId:orderID});
